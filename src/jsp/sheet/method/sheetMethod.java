...
public class sheetMethod {
	...
	public static void synchronization() throws GeneralSecurityException, IOException, ServiceException {
		...
		for(int i=0; i<pjList.size(); i++) {
			...
			li.getCustomElements().setValueLocal("팀수주", pjList.get(i).getTEAM_ORDER());
			li.getCustomElements().setValueLocal("팀매출", pjList.get(i).getTEAM_SALES());
			...
			li.getCustomElements().setValueLocal("프로젝트계약금액백만", String.valueOf(pjList.get(i).getPROJECT_DESOPIT()));
			li.getCustomElements().setValueLocal("상반기예상수주", String.valueOf(pjList.get(i).getFH_ORDER_PROJECTIONS()));
			li.getCustomElements().setValueLocal("상반기수주", String.valueOf(pjList.get(i).getFH_ORDER()));
			li.getCustomElements().setValueLocal("상반기예상매출", String.valueOf(pjList.get(i).getFH_SALES_PROJECTIONS()));
			li.getCustomElements().setValueLocal("상반기매출", String.valueOf(pjList.get(i).getFH_SALES()));
			li.getCustomElements().setValueLocal("하반기예상수주", String.valueOf(pjList.get(i).getSH_ORDER_PROJECTIONS()));
			li.getCustomElements().setValueLocal("하반기수주", String.valueOf(pjList.get(i).getSH_ORDER()));
			li.getCustomElements().setValueLocal("하반기예상매출", String.valueOf(pjList.get(i).getSH_SALES_PROJECTIONS()));
			li.getCustomElements().setValueLocal("하반기매출", String.valueOf(pjList.get(i).getSH_SALES()));
			li.getCustomElements().setValueLocal("착수", pjList.get(i).getPROJECT_START());
			li.getCustomElements().setValueLocal("종료", pjList.get(i).getPROJECT_END());
			li.getCustomElements().setValueLocal("고객담당자", pjList.get(i).getCLIENT_PTB());
			li.getCustomElements().setValueLocal("근무지", pjList.get(i).getWORK_PLACE());
