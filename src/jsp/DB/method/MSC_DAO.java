...
public class MSC_DAO {
	...
	public int insert_MSC (String id, String amPlace, String pmPlace, String date) {
		...
		try {
				conn = DBconnection.getConnection();
				pstmt = conn.prepareStatement("insert into manager_schedule(ID, 날짜, 오전장소, 오후장소) "
						+ "values(?, ?, ?, ?)");
				pstmt.setString(1, id);
				pstmt.setString(2, date);
	...
	// 관리자 일주일 일정 추가
	public ArrayList<String> weekAdd_MSC(String[] AmPlace_arr, String[] PmPlace_arr, String[] date_arr, String sessionID) {
